...
start_wl(void)
{
	...
	if(region == 4 || region == 9 || region == 11){
        ...
        system("wl -i eth1 txcore -k 7 -o 7 -s 1 -c 7 -s 2 -c 7");
		if(atoi(nvram_get("wlg_channel")) >= 36 && atoi(nvram_get("wlg_channel")) <= 48)
		{
			eval("wl", "-i", "eth2", "country", "EU/62");
		}else if(atoi(nvram_get("wlg_channel")) >=149 && atoi(nvram_get("wlg_channel")) <= 161)
		{
		    eval("wl", "-i", "eth2", "country", "Q2/42");
		}
		system("wl -i eth2 txcore -o 7 -s 1 -c 7 -s 2 -c 7 -s 3 -c 7");
	}else if(region == 14) /*Russia*/
	{
	    eval("wl", "country", "EU/62");
        system("wl -i eth1 txcore -k 7 -o 7 -s 1 -c 7 -s 2 -c 7");
    ...
    /* Add this to fix 2.4G issue, same as R6200v2 */
    eval("wl", "mimo_ss_stf", "1");
#else
	if(region == 4 || region == 9 || region == 11){
		...
		eval("wl", "-i", "eth2", "country", "TW/1");
		system("wl -i eth2 txcore -o 7 -s 1 -c 7 -s 2 -c 7 -s 3 -c 7");
		/*change country code when region is RU & AU*/
	}else if(region == 3){
